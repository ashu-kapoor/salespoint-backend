...
public class SalesQueryService {
  ...
  QueryProducer<Document> producer = new QueryProducer<>("cdc-sales-query");

  public void listenToSales() {
  ...
  private void sendToKafka(Document entity) {
    log.info("Triggering sales query kafka with topic:{}, payload:{}", "SALES_QUERY", entity);
    producer
        .getSender()
        .send(
            Flux.just(
                SenderRecord.create(
                    new ProducerRecord<>("SALES_QUERY", entity.get("_id").toString(), entity),
                    entity.get("_id").toString())))
        .doOnNext(
            senderResult ->
                log.info(
                    "send record topic:{}, sales:{}",
