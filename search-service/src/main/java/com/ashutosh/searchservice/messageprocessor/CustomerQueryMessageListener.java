...
public class CustomerQueryMessageListener {

  private final CustomerQueryService customerQueryService;
  private final QueryReceiver<Document> sagaReceiver =
      new QueryReceiver<>("CUSTOMER_QUERY", "customer-query");
        ...
        .doOnNext(
            r -> {
              log.info(
                  ...
                  r.topic(),
                  r.partition());
              log.info("Customer query processing Saga payload {}", r.value());
            })
        .flatMap(record -> customerQueryService.processRecord(record.value(), record.key()))
