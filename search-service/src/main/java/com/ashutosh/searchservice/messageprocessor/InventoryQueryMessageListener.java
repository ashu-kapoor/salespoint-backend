...
public class InventoryQueryMessageListener {

  private final InventoryQueryService inventoryQueryService;
  private final QueryReceiver<Document> sagaReceiver =
      new QueryReceiver<>("INVENTORY_QUERY", "inventory-query");
        ...
        .doOnNext(
            r -> {
              log.info(
                  ...
                  r.topic(),
                  r.partition());
              log.info("Inventory query processing Saga payload {}", r.value());
            })
        .flatMap(record -> inventoryQueryService.processRecord(record.value(), record.key()))
